...
- Kurva fling ala iOS (Batch 364, +3 layar Batch 367, +3 layar Batch 377, +3 layar Batch 378, +3 layar Batch 379) baru dipasang di 13 dari 15 layar berscroll (tab Lagu di `LibraryScreen.kt`, `HomeScreen`, `PlaylistScreen`, `QueueSheet`, `SmartPlaylistScreen`, `SettingsScreen`, `StatsDashboardScreen`, `VaultSheet`, `SongPickerSheet`, `FolderManagerSheet`, `DuplicateFinderSheet`, `ABRepeatBookmarkSheet`, `EqualizerSheet`) — 2 sisanya (`LyricsSheet`, `LyricsView`) masih pakai kurva fling default Android — daftar lengkap & pola pemasangannya di `PENDING_IosFlingBehavior.md`. Rubber-band overscroll-nya sendiri **sudah** otomatis app-wide di semua layar (dipasang sekali di root `Theme.kt`, bukan per-layar)
