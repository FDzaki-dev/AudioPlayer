...
- **4 tema**: keluarga utama gaya Apple (ikuti sistem, terang, gelap — palet iOS system color, sudut membulat generous) plus satu identitas custom, **Tactile**, murni dari spesifikasi desain skeuomorphism-lite literal Midnight Blue yang di-supply user (`compose-skeuomorphism-lite-midnight-blue.md`, Batch 52 — menggantikan varian hybrid-glass Batch 51, AMOLED-hitam Batch 50, varian terang Batch 49, & Matte Noir sebelumnya). Wajib gelap (bukan ikut sistem, selalu gelap), latar literal Midnight Blue (`#191970`) rata/opak (bukan gradient, bukan kaca translusen lagi), aksen biru-ungu, bevel/bayangan gambar-tangan (bukan aset tekstur bitmap) lewat satu helper terpusat `tactileEmboss()` — bukan sekadar ganti warna datar. Border & glow sengaja direstrain (alpha rendah, literal 0.055/0.035/0.65 dari spec) mengikuti aturan spesifikasi "jangan pakai border putih terang"; sliders/toggles/switches tetap komponen Material3 polos (di luar cakupan batch ini).
