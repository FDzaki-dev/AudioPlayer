...
- **4 tema**: keluarga utama gaya Apple (ikuti sistem, terang, gelap — palet iOS system color, sudut membulat generous) plus satu identitas custom, **Tactile**, murni dari spesifikasi desain skeuomorphism-lite yang di-supply user (`compose-skeuomorphism-lite-dark.md`, Batch 50 — menggantikan varian terang Batch 49 & Matte Noir sebelumnya). Wajib gelap/AMOLED-first (bukan ikut sistem, selalu gelap), aksen biru dingin, permukaan panel dibangun dari gradient + bevel border + bayangan gambar-tangan (bukan aset tekstur bitmap) lewat satu helper terpusat `tactileEmboss()` — bukan sekadar ganti warna datar. Border & glow sengaja direstrain (alpha rendah) mengikuti aturan spesifikasi "jangan pakai border putih terang"; sliders/toggles/switches tetap komponen Material3 polos (di luar cakupan batch ini).
