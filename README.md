...
> 🆕 **Update terbaru — Batch 314 (Fix sheet "Kontrol Lanjutan" terpotong + Equalizer tidak auto re-attach ke sesi audio baru):** 2 laporan user. (1) `AdvancedControlsSheet` sekarang bisa discroll (`verticalScroll`) — sebelumnya baris paling bawah ("Potong Nada Dering") ke-clip diam-diam di layar pendek/font besar; 5 sheet lain dengan pola sama diaudit & diantrekan Batch 315. (2) Equalizer kini auto re-attach ke sesi audio BARU (cold-start/Service restart/lagu baru) lewat hook `PlaybackAudioSession.onSessionIdChanged` — sebelumnya cuma ter-reattach kalau sheet Equalizer dibuka manual, jadi settingan tersimpan tapi suara sempat flat sampai sheet dibuka lagi.
...
- Diantrekan Batch 315: `verticalScroll` (fix pola sama Batch 314) belum diterapkan ke `EqualizerSheet.kt`, `RingtoneCutterSheet.kt`, `VisualizerSheet.kt`, `UpdateCheckSheet.kt`, `BackupRestoreSheet.kt` — hasil audit "pola tab serupa" Batch 314, berpotensi ikut terpotong di layar pendek/font besar sama seperti "Kontrol Lanjutan"
