...
- **4 tema**: keluarga utama gaya Apple (ikuti sistem, terang, gelap — palet iOS system color, sudut membulat generous) plus satu identitas custom, **Tactile**, murni dari spesifikasi desain skeuomorphism-lite hybrid-glass yang di-supply user (`compose-skeuomorphism-lite-hybrid-glass-dark-blue.md`, Batch 51 — menggantikan varian AMOLED-hitam Batch 50, varian terang Batch 49, & Matte Noir sebelumnya). Wajib gelap (bukan ikut sistem, selalu gelap) tapi BUKAN AMOLED-black murni — atmosfer gradient navy→biru-gelap diagonal di root layar, permukaan panel translusen ("kaca") yang menembus gradient itu, aksen biru dingin, bevel/bayangan gambar-tangan (bukan aset tekstur bitmap) lewat satu helper terpusat `tactileEmboss()` — bukan sekadar ganti warna datar. Border & glow sengaja direstrain (alpha rendah) mengikuti aturan spesifikasi "jangan pakai border putih terang"; sliders/toggles/switches tetap komponen Material3 polos (di luar cakupan batch ini).
