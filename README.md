...
>   Batch 448 valid. Masih belum dikonfirmasi: label 3 tab tidak terpotong di font-scale
>   aksesibilitas besar; drag jari lintas tab bar (termasuk lintas >1 batas tab) berpindah tab dgn
...
> - **BARU (Batch 449)**: user device-test Batch 448 menemukan kilatan kotak abu-abu ~0.25 detik
>   di tab yang BARU DITINGGALKAN tiap pindah tab — root cause `NavigationBarItem` M3 (state-layer
>   internalnya TIDAK benar-benar mati oleh `NoRippleIndication` Batch 439, ternyata cuma override
>   `LocalIndication` yang tidak pernah dibaca komponen itu). Fix: `NavigationBarItem` DIHAPUS
>   TOTAL dari bottom nav, diganti `CustomNavBarTabItem` (Row+Box manual + `Modifier.selectable
>   (indication = null, ...)`, kontrak resmi Compose Foundation). Belum dikonfirmasi device: 0
>   lagi kilatan abu-abu di tab yang ditinggalkan; 0 regresi warna ikon/label tema Skeu (dulu
>   implisit lewat `LocalContentColor` NavigationBarItem, kini eksplisit snap token M3 resmi); 0
>   regresi klik/routing/`bouncyPress`/aksesibilitas TalkBack (role Tab).
