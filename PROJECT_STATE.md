...
pada instruksi generik), bukan reopen. **Status DISCONTINUED tetap permanen tidak diubah** (final
lock Batch 410). Tidak ada ZIP baru dari user sesi ini (upload terakhir tetap
`AudioPlayer_v416.zip`).

Audit: cakupan unit test pure-JVM (`app/src/test`, 0 Robolectric). `DuplicateDetector.kt` (Gap
List #2) ketemu sbg satu-satunya kelas pure di codebase yang belum punya test file, walau KDoc-nya
sendiri eksplisit bilang "trivially unit-testable". Kelas pure lain semua sudah tertest — 0
kandidat lain. File baru: `DuplicateDetectorTest.kt` (12 test — `findLibraryDuplicates` &
`findPhysicalDuplicates`, termasuk boundary duration tepat di detik penuh, fileSize<=0
exclusion, sorting descending, & 1 test independensi silang kedua fungsi). Fixture `song()` pakai
pola persis `SmartPlaylistEngineTest` (`mock(Uri::class.java)`, 0 dependency baru — mockito-core
sudah ada sejak Batch 27 revisi 2). Brace/paren balance file baru (12/12, 101/101). **0 file kode
app existing disentuh** — `DuplicateDetector.kt` sendiri 0 diubah, ZERO-REFACTOR aman by
construction. **Belum pernah dijalankan compiler sungguhan** (lingkungan kerja sesi ini 0 Gradle/
Android SDK, batasan lama yang sama) — verifikasi sungguhan lewat CI `testDebugUnitTest` begitu
di-push. `FILE_MANIFEST.txt` diupdate 189→190. `README.md` § Testing +1 kalimat. Detail lengkap
`CHANGELOG.md` Batch 417.

**Scope**: 1 file kode baru, 0 file kode existing diubah. 4 file dokumentasi (`PROJECT_STATE.md`,
`CHANGELOG.md`, `README.md`, `FILE_MANIFEST.txt`).

**Batch 416 (Sektor optimasi Compose Batch 392-415 resmi DITUTUP, instruksi eksplisit user, 0
kode)** — User: *"Tutup sektor optimasi Compose"* (dipilih dari 4 opsi setelah Batch 415
melaporkan 0 kandidat baru). **Status DISCONTINUED tetap permanen tidak diubah** (final lock
Batch 410 — ini penutupan sektor kerja teknis, BUKAN status proyek). Tidak ada ZIP baru sesi ini.

**Ringkasan 24 batch (392-415)**: `identityRootBrush`/`Brush.*Gradient` remember sweep (392-401,
"habis" per kesimpulan 401) → `SDK_INT` legacy dead-code TUNTAS 11/11 (402-408) →
`collectAsState()` lifecycle fix 2 titik (409) → Regex hoist + audit `LaunchedEffect` negatif
(412) → `List<Song>` stability via config, 0 kode (413) → hoist `listOf()` literal (414) → 3
kelas audit negatif: filter/sort/groupBy-tanpa-remember, `.values()` vs `.entries`, `Set` vs
`List` contains (415) → 1 kelas lagi dicoba batch ini: `MutableInteractionSource()` tanpa
remember — 35 titik, 100% sudah benar, NEGATIF juga.

**Tetap utang teknis, BUKAN "selesai"**: `AlbumArt` `SubcomposeAsyncImage` — perlu ganti slot
`error`/`placeholder` dari `@Composable` ke `Painter` (`AsyncImage` biasa), mengubah mekanisme
pengukuran/gambar secara struktural, TIDAK bisa dibuktikan zero-visual-difference murni dari
kode — butuh device fisik. Ini SATU-SATUNYA alasan penutupan, bukan kehabisan effort (4 kelas
audit dicoba tuntas, genuinely nihil).

**Aturan baru (item 7 di atas)**: sesi berikutnya JANGAN otomatis buka lagi sektor ini pada
instruksi generik "next"/"lanjut" — TAPI kalau user secara eksplisit minta ("buka lagi sektor
Compose", "coba lagi AlbumArt", dst.), boleh dieksekusi normal. Detail lengkap CHANGELOG.md
Batch 416.

**Scope**: 0 file kode. 3 file dokumentasi (`PROJECT_STATE.md`, `CHANGELOG.md`, `README.md`).
Diff eksplisit vs ZIP Batch 415 dikonfirmasi cuma 3 file dokumentasi ini berubah.

**Batch 415 (Audit 3 kelas alokasi/lookup app-wide: hasil NEGATIF, 0 kode diubah)** — User:
*"next"*. **Status DISCONTINUED tetap permanen tidak diubah** (final lock Batch 410). Tidak ada
ZIP baru sesi ini (upload terakhir tetap `AudioPlayer_v412.zip`); lanjut dari state kerja Batch
414.

Sektor `listOf(...)` literal (Batch 414) TUNTAS. 3 kelas audit baru dicoba (belum pernah digrep
sistematis sebelumnya): **(1)** `.filter{}`/`.sortedBy{}`/`.groupBy{}` di badan `@Composable`
tanpa `remember` — 27 titik app-wide diperiksa satu-per-satu, 100% sudah benar (mayoritas sudah
`remember(...)` dgn key tepat tapi tidak kelihatan di grep single-line krn `remember(...) {`-nya
beberapa baris di atas; sisanya di fungsi biasa/lambda event handler, bukan per-recomposition).
**(2)** `EnumClass.values()` (alokasi array tiap panggilan) vs `.entries` (cached) — 0 usage
`.values()` app-wide, sudah konsisten `.entries`. **(3)** `Set` vs `List` utk `.contains()` di
filter ID lagu (O(1) vs O(n), signifikan utk library ribuan lagu) — seluruh koleksi ID hot-path
(`hiddenIds`, `vaultedIds`, `excludedFolders`, `alreadyAddedIds`, `favoriteIds`/`selectedIds`)
dikonfirmasi sudah `Set<Long>`/`Set<String>`. Tambahan cek singkat: `SimpleDateFormat`/
`DateTimeFormatter` — 0 usage app-wide (tidak relevan, proyek tidak format tanggal manual).

**Kesimpulan jujur**: ketiga kelas NIHIL app-wide, codebase genuinely sudah bersih (konsisten
21+ batch optimasi Compose sebelumnya). **0 kode diubah** — dicatat eksplisit supaya sesi
berikutnya tidak mengulang audit yang sama. Detail lengkap CHANGELOG.md Batch 415.

**Kandidat blocked Batch 392 — status tidak berubah**: `List<Song>` TUNTAS (Batch 413). Sisa 1:
`AlbumArt` `SubcomposeAsyncImage`, tetap diblokir (butuh device fisik) — tidak diperiksa ulang
batch ini, status sama seperti akhir Batch 414.

**Sesi berikutnya**: 0 kandidat low-risk baru tersisa dari kelas yang sudah dicoba. Opsi realistis:
(a) `AlbumArt` (butuh user sediakan verifikasi visual/device fisik), (b) kelas audit lain yang
belum dicoba sesi manapun (mis. Macrobenchmark/profiling device asli — dari awal butuh data
device, bukan dari kode, per Batch 388), atau (c) arah baru di luar sektor optimasi.
...
pilih arah baru). **Status DISCONTINUED tetap permanen tidak diubah** (Batch 387 — "optimasi
murni"). TIDAK ada ZIP baru sesi ini (upload terakhir tetap `AudioPlayer_v408.zip`); lanjut dari
state kerja sesi.

**Temuan**: proyek konsisten pakai `collectAsStateWithLifecycle()` (dependency
`lifecycle-runtime-compose:2.8.2` sudah ada) — 41 titik app-wide SUDAH benar
(`MainActivity.kt` 33, `NowPlayingScreen.kt` 4, dst). 0 sesi sebelumnya pernah grep khusus cari
OUTLIER `collectAsState()` polos — grep app-wide `\.collectAsState()\b` (regex terpisah dari
`collectAsStateWithLifecycle`) menemukan **tepat 2 titik kelewat**, pas di bawah batas 3-file.

**2 titik fix**: (1) `NowPlayingScreen.kt:268` — `lyricsAutoState`
(`StateFlow<LyricsUiState>` dari `LyricsViewModel.uiState`) → `collectAsStateWithLifecycle()`,
import sudah ada di file yang sama (dipakai 4 titik lain). (2) `UpdateCheckSheet.kt:31` — `state`
(`StateFlow<UpdateState>` dari `UpdateManager.state`, singleton `object`) →
`collectAsStateWithLifecycle()`, import baru ditambahkan.

**Kelas bug**: `collectAsState()` polos tetap collect Flow walau lifecycle di bawah `STARTED`
(app background) — buang CPU/battery. `collectAsStateWithLifecycle()` (default `STARTED`) pause
otomatis saat `STOPPED`, resume transparan (StateFlow re-emit `.value` terbaru, 0 event hilang).

**Verifikasi**: kedua properti dikonfirmasi genuinely `StateFlow` (bukan `SharedFlow`/`Flow`
biasa). 0 pemanggil lain di luar 2 titik ini (grep app-wide). **Zero behavior change** selagi app
foreground — beda perilaku HANYA saat background (manfaat, bukan regresi). Balance kurung dicek
programatis (Python, string/comment-aware): `NowPlayingScreen.kt` 282/282 `{}` 789/789 `()`;
`UpdateCheckSheet.kt` 22/22 `{}` 61/61 `()` — seimbang. Diff eksplisit vs ZIP Batch 408
dikonfirmasi CUMA 2 file ini berubah, 0 file lain kesenggol. Detail lengkap CHANGELOG.md Batch 409.

**Belum diverifikasi build/runtime sungguhan** (0 kotlinc/SDK/network di sandbox) — WAJIB cek
GitHub Actions setelah push. Risiko dinilai LEBIH RENDAH dari sektor sebelumnya krn murni ganti
1 API resmi AndroidX dgn API resmi AndroidX lain (drop-in).

**Sektor `collectAsState()` non-lifecycle-aware — TUNTAS** (0 sisa titik app-wide, dikonfirmasi
grep ulang). **Sesi berikutnya**: kandidat baru yang belum pernah teraudit — (a) `Regex(...)`
dibangun ulang tiap panggilan di `RingtoneEncoder.kt:150` (bukan hot-path, prioritas rendah);
(b) audit `LaunchedEffect(key)` app-wide vs key yang tepat (belum pernah digrep sistematis). 2
kandidat lama (`AlbumArt` `SubcomposeAsyncImage`, stabilitas `List<Song>`) TETAP diblokir sejak
Batch 392, status tidak berubah.

**Batch 408 (Sektor `SDK_INT` legacy mati, file terakhir: `PlaybackService.kt`, 1 file kode —
SEKTOR TUNTAS 11/11)** — User: "next" (lanjutan Batch 407, sektor sama, TANPA ZIP baru — lihat
catatan integritas di bawah). **Status DISCONTINUED tetap permanen tidak diubah** (per klarifikasi
Batch 387 — kategori "optimasi murni").

**Catatan integritas alur kerja**: sesi ini TIDAK menerima ZIP baru dari user (upload terakhir
tetap `AudioPlayer_v406.zip`) — instruksi "next" datang langsung setelah output Batch 407 di
percakapan yang sama, sebelum user sempat menjalankan skrip Termux [DAILY UPDATE]. Batch ini
melanjutkan dari state kerja sesi (`AudioPlayer_v407.zip` yang baru dikirim), BUKAN dari ZIP user
yang lebih baru — dicatat eksplisit di sini krn aturan "ZIP dari user = sumber kebenaran mutlak"
biasanya mengasumsikan ZIP baru tiap batch. Kalau user ternyata sempat push/edit manual di luar
ZIP yang diproses sesi ini sebelum instruksi "next" ini terkirim, state itu TIDAK tercermin di
batch ini — sesi berikutnya yang menerima ZIP baru wajib diff dulu terhadap asumsi ini.

Dibaca penuh dulu (840 baris) sebelum eksekusi, sesuai peringatan eksplisit Batch 402/405/406/407
(file ini masuk "paling berisiko" § arsitektur, plus kekhawatiran khusus soal
`SongArtBitmapLoader` yang dipanggil dari background thread Media3). **Temuan penting: katalog
Batch 402 mencatat "5 titik M/O/O/Q/Q" utk file ini — grep ulang app-wide+baca penuh sesi ini
menemukan SEBENARNYA 7 titik** (1×M, 3×O, 3×Q — bukan 1×M/2×O/2×Q). Dicatat jujur di sini
(bukan disembunyikan/diselaraskan diam-diam ke angka lama) — kemungkinan besar audit awal Batch
402 cuma grep pattern tanpa membaca tiap fungsi penuh utk file "berisiko" ini (sesuai catatan
batch itu sendiri: "SENGAJA BELUM disentuh krn butuh baca konteks penuh dulu"), jadi undercount
wajar terjadi, bukan indikasi ada yang salah di kode.

**7 titik dieksekusi (semua di `PlaybackService.kt`)**:
1. `maybeStartFloatingBubble()` — guard majemuk `SDK_INT >= M(23) && !canDrawOverlays()` →
   `SDK_INT` selalu true di `minSdk` 31, disederhanakan jadi `!canDrawOverlays()` saja.
2. `maybeStartFloatingBubble()` — if/else `SDK_INT >= O(26)` → selalu `startForegroundService()`.
3. `buildColdStartNotification()` — `togglePendingIntent` if/else `SDK_INT >= O(26)` → selalu
   `PendingIntent.getForegroundService(...)`.
4. `updateColdStartNotification()` — if/else `SDK_INT >= Q(29)` → selalu `startForeground(...,
   FOREGROUND_SERVICE_TYPE_MEDIA_PLAYBACK)`.
5. `startForegroundColdStartNotification()` — guard `SDK_INT >= O(26)` yang membungkus pembuatan
   notification channel → selalu true di `minSdk` 31, dihapus (channel selalu dibuat, bukan
   dihapus channel-nya — cuma guard-nya yang redundant).
6. `startForegroundColdStartNotification()` — if/else `SDK_INT >= Q(29)` (pola identik #4, fungsi
   berbeda) → selalu `startForeground(..., FOREGROUND_SERVICE_TYPE_MEDIA_PLAYBACK)`.
7. `SongArtBitmapLoader.loadBitmap()` — if/else `SDK_INT >= Q(29)` (pola identik ke
   `AccentColorExtractor.kt`/`AudioArtFetcher.kt`/`WidgetUpdater.kt` Batch 402: `loadThumbnail()`
   vs fallback manual `BitmapFactory.decodeStream`) → selalu `loadThumbnail(uri, Size(512,512),
   null)`. **Soal kekhawatiran "background thread Media3"**: dikonfirmasi cek eksekusi tetap di
   `scope.launch(Dispatchers.IO)` yang SAMA, tidak berubah sama sekali — SDK_INT check itu sendiri
   cuma boolean sinkron biasa, 0 hubungan dgn threading; kekhawatiran katalog Batch 402/406 valid
   sbg alasan "baca detail dulu" tapi TIDAK menghasilkan risiko tambahan di fix aktualnya.

**Verifikasi app-wide sebelum eksekusi**: 2 file `androidTest`
(`PlaybackServiceTestHelper.kt`/`PlaybackTransportTest.kt`) digrep utk 6 simbol yang disentuh —
0 match, konfirmasi tidak ada test yang menguji fungsi-fungsi ini (test transport murni, bukan
foreground-service/notification/bitmap-loading). `SongArtBitmapLoader` dikonfirmasi 0 dirujuk
file lain (`private class`, cuma dipakai internal file ini). Import `Build` dihapus (0 pemakaian
tersisa setelah ketujuh titik di atas — dicek eksplisit, file ini 0 titik `TIRAMISU`/`S_V2`/
`UPSIDE_DOWN_CAKE` yang perlu dipertahankan, beda dari `MainActivity.kt`/`FloatingBubbleService.kt`).

**Zero behavior change** di ketujuh titik: kondisi yang dihapus SELALU true di `minSdk` 31
manapun (dijamin OS, bukan asumsi) — jalur eksekusi runtime 100% identik sebelum/sesudah, termasuk
thread/dispatcher di titik 7. Balance kurung dicek programatis (78/78 `{}`, 258/258 `()`, 1/1
`[]`, seimbang), diff eksplisit vs ZIP Batch 407 dikonfirmasi cuma 1 file kode ini berubah
(840 → 819 baris, turun 21 baris net). Belum diverifikasi build/runtime sungguhan (0 kotlinc/SDK/
network di sandbox, sama seperti seluruh rangkaian batch sektor ini) — titik #7
(`SongArtBitmapLoader`) & foreground-service start (#2/#3/#4/#5/#6) layak diverifikasi device
fisik ekstra teliti (artwork notifikasi/lock-screen tetap termuat, cold-start widget tetap jadi
foreground service tanpa crash `ForegroundServiceStartNotAllowedException`) krn file ini memang
ditandai paling berisiko sejak awal sektor ini. Detail lengkap CHANGELOG.md Batch 408.

**Sektor `SDK_INT` legacy — status setelah Batch 408: TUNTAS 11/11 file.** Seluruh cabang mati
`Build.VERSION_CODES` di bawah `minSdk` 31 (M/N/O/P/Q/R, katalog app-wide Batch 402) sudah
diaudit & dibersihkan lintas 8 sesi (Batch 402-408), 0 sisa yang diketahui. Titik `S_V2`(32)/
`TIRAMISU`(33)/`UPSIDE_DOWN_CAKE`(34) di `FloatingBubbleService.kt`/`BubbleTileService.kt`/
`MainActivity.kt` SENGAJA dipertahankan (masih relevan, level API-nya > `minSdk` 31). **Sesi
berikutnya**: sektor ini resmi ditutup, kembali ke kandidat Compose yang diblokir sejak Batch 392
(`AlbumArt` `SubcomposeAsyncImage` — 6 titik app-wide, risiko regresi tint dinamis; stabilitas
`List<Song>` app-wide — lintas banyak file, melampaui batas 3-file/task) — ATAU tanya user arah
baru kalau kedua kandidat itu masih dianggap terlalu berisiko tanpa device fisik.
...
Batch 398 menutup kelas bug "`Brush.*Gradient()` LANGSUNG di badan sebuah screen tanpa
`remember`" (0 sisa titik provably-bug app-wide). Batch ini membuka sudut audit baru yang belum
pernah digrep eksplisit sebelumnya: fungsi Modifier **extension** di `theme/TactileDepth.kt`
(`calmAberration()`/`calmScanlines()`/`calmGrain()` — pola drawBehind/drawWithContent, beda
mekanisme dari `embossSurface()`/`frostedGlass()` yang sudah dituntaskan Batch 395/396) yang
dipanggil ulang dari scope panas MiniPlayerBar — grep `Brush.*Gradient()` app-wide sebelumnya
TIDAK menangkap ini karena occurrence-nya ada di DALAM fungsi shared di `TactileDepth.kt`, bukan
langsung di badan file screen manapun.

**Root cause**: `calmAberration()` (efek "aberrasi kromatik" 2-lingkaran radial-gradient,
identitas Calm Retro) dipanggil dari `MiniPlayerBar.kt` (`isCalmRetro -> Modifier.calmAberration(
bias = 2.dp)` pada tombol Play/Pause). MiniPlayerBar sengaja koleksi `playbackProgress` LOKAL
tiap detik selama musik main (desain Batch 353) — SELURUH badan fungsinya (termasuk `when` yang
memanggil `calmAberration()`) recompose 1x/detik untuk user yang pakai identitas Calm Retro.
Sebelum fix ini, tiap tick memanggil ulang `this.drawBehind { ... }` — lambda BARU (capture
`bias`) tiap panggilan, artinya 1 instance `DrawBehindElement` baru (2x `Brush.radialGradient`
di dalamnya) dibangun tiap detik, padahal `bias` itu sendiri konstan (literal `2.dp` di
MiniPlayerBar, `3.dp` default di 2 pemanggil lain — `NowPlayingScreen.kt`/`SettingsScreen.kt`,
keduanya TIDAK dalam scope tick 1x/detik sejak Batch 353 memisahkan posisi/durasi keluar dari
body composable besar `NowPlayingScreen`).

**Fix**: Modifier hasil `drawBehind { ... }` dibungkus `remember(bias)` di dalam fungsi
`calmAberration()` sendiri (`TactileDepth.kt`) — begitu `bias` sama (kasus SEMUA 3 pemanggil,
tiap panggilan pakai literal `Dp` tetap), instance Modifier yang SAMA dipakai ulang lintas
recomposition, alih-alih dibangun dari nol tiap tick. `size`/`center`/`radius` di dalam lambda
draw TETAP dihitung fresh tiap draw call sungguhan (`DrawScope`, nilai yang genuinely bergantung
ukuran layout aktual — bukan sesuatu yang bisa/perlu ikut di-remember) — cuma WADAH Modifier-nya
yang sekarang stabil. 0 pemanggilan `@Composable` di dalam lambda `remember` (`Modifier.drawBehind`
adalah fungsi biasa, bukan `@Composable` — dicek eksplisit, tidak mengulang kesalahan
`@DisallowComposableCalls` Batch 393). **Zero behavior change** — 2 lingkaran radial-gradient
aberrasi tetap identik visual & posisi persis, cuma alokasi objek berkurang saat `bias` tidak
berubah (mayoritas waktu playback untuk user Calm Retro).

**1 file kode disentuh** (`TactileDepth.kt`, 515->537 baris). Brace/paren balance seimbang
(33/33 `{}`, 146/146 `()`, dicek python3 tokenizer string/comment-aware sebelum & sesudah edit).
0 pemanggil (`MiniPlayerBar.kt`/`NowPlayingScreen.kt`/`SettingsScreen.kt`) perlu diubah — signature
`calmAberration(bias: Dp = 3.dp): Modifier` 100% tidak berubah. Masih belum ditest build/lint
sungguhan — rekomendasi ke user: push & jalankan CI, lalu verifikasi visual device fisik untuk
identitas Calm Retro (tombol Play/Pause MiniPlayerBar + 2 pemanggil lain) — pastikan efek aberrasi
kromatik tetap identik, 0 stutter baru. Sisa kandidat compose sector: `AlbumArt` SubcomposeAsyncImage
(butuh Painter, tidak bisa drop-in pertahankan tint dinamis tanpa regresi visual, 6 titik app-wide)
dan stabilitas `List<Song>` app-wide (perlu ubah signature Composable lintas banyak file, melampaui
batas 3-file/task) — keduanya BELUM dieksekusi, sama alasan sejak Batch 392. `calmScanlines()`/
`calmGrain()` (2 fungsi bertetangga di file yang sama) diperiksa juga: 0 call site keduanya berada
di scope tick 1x/detik (`calmGrain()` cuma dipanggil sekali di root Surface `MainActivity.kt`,
`calmScanlines()` dipanggil di `SongRow`/sheet lain yang recompose per-aksi bukan per-detik) —
SENGAJA TIDAK disentuh, bukan bug di kelas yang sama. Detail lengkap CHANGELOG.md Batch 399.
...
"repack" tanpa laporan bug atau kata kerja lanjutan ("lanjut"/"next"/dsb.) — BUKAN instruksi kerja
baru sesuai definisi banner "ATURAN SESI AKTIF" di atas (bukan pula kategori "optimasi murni"
Batch 385-389), jadi **status DISCONTINUED TIDAK berubah/TIDAK perlu dievaluasi ulang** — repack
verifikasi ini netral terhadap status proyek, sama seperti Batch 320 dulu.

**Cakupan verifikasi (lebih luas dari cek per-file biasa, konsisten metodologi Batch 320)**:
1. **Diff penuh `FILE_MANIFEST.txt` vs isi ZIP** (bukan cuma spot-check file yang baru disentuh
   batch sebelumnya) — **1 drift NYATA ditemukan**: `ui/theme/IosScrollPhysics.kt` (dibuat Batch
   364, salah satu dari 3 file kode batch itu) TERNYATA TIDAK PERNAH ditambahkan ke
   `FILE_MANIFEST.txt` — lolos dari audit manapun sejak Batch 364 sampai sekarang (26 batch).
   **Fix**: entri ditambahkan di posisi alfabetis yang benar, total dikoreksi 188→189, catatan
   penjelasan ditambahkan di header file itu sendiri. `.github/workflows/build.yml` &
   `.gitignore` TETAP tercatat di manifest (representasi `git ls-files` yang sebenarnya) walau
   TIDAK ikut dikirim di ZIP sesi ini — **ini BUKAN drift**, sesuai desain skrip Termux [DAILY
   UPDATE] (P5 GIT, lihat skrip di bawah) yang sengaja mengecualikan `-name '.*'` dari langkah
   hapus-isi-folder, jadi 2 file itu memang dikelola permanen di device, bukan lewat siklus ZIP
   tiap sesi.
2. **Brace/paren/bracket balance SEMUA 127 file `.kt`** (bukan cuma file yang baru disentuh) —
   tokenizer single-pass string/comment/char-literal-aware (python3, metodologi sama yang dipakai
   batch-batch sebelumnya) dijalankan ke SELURUH codebase: **0/127 file timpang**. 3 file
   `.kts` (`build.gradle.kts`, `settings.gradle.kts`, `app/build.gradle.kts`) dicek terpisah:
   semua seimbang.
3. **Validitas XML** — seluruh 22 file `.xml` (manifest, layout, drawable, xml/) di-parse
   (`xml.etree.ElementTree`): 0 error, semua valid.

**Hasil: 189/189 file akun (setelah fix #1), 0 file kode diubah, 0 bug kode ditemukan.** ZIP
direpack identik isi kode dengan Batch 389 — cuma `FILE_MANIFEST.txt` yang dikoreksi (file
tracking/dokumentasi, bukan kode aplikasi, jadi TIDAK menghitung ke batas Micro-Batch 3-file-
kode). Versi APK tetap auto dari `GITHUB_RUN_NUMBER` (0 bump manual, konsisten Rule 1 § "Aturan
sesi: transparansi versi").

**Belum divalidasi compile Gradle sungguhan** (0 akses SDK/Gradle/jaringan di sandbox sesi ini —
sama seperti semua batch sebelumnya) — TAPI batch ini 0 menyentuh kode Kotlin/XML app sama sekali,
jadi 0 risiko regresi compile baru dari sesi ini sendiri (state compile-risk identik ke Batch 389).

**Batch 389 (Optimasi cold-start — overlapPlayer/CrossfadeEngine lazy init, `PlaybackService.kt`,
1 file kode)** — User instruksi: "lanjut progress optimize!!" (lanjutan sesi optimasi yang sama,
Batch 385→386→387→388→389). **Status DISCONTINUED tetap permanen tidak diubah** (per klarifikasi
Batch 387).

Batch 388 menandai "setup ExoPlayer/MediaSession di `PlaybackService`" sebagai kandidat yang
belum diperiksa detail (baru disebut butuh data device, bukan sudah diaudit tuntas) — batch ini
masuk lebih dalam ke titik spesifik itu, bukan menebak ulang di area cold-start
Application/MainActivity yang Batch 388 sudah nyatakan exhausted.
...
Batch 102) — unconditional, walau `CrossfadeStore.isEnabled()` default `false` (opt-in, lihat
komentar kelasnya sendiri). Di config default, `overlapPlayer` (ExoPlayer kedua lengkap dgn
renderer/decoder/AudioTrack native) dibangun tiap cold start padahal
`CrossfadeEngine.maybeStartCrossfade()` sendiri `return` di baris pertama kalau `!enabled` —
digrep, tidak ada jalur lain yang menyentuh `overlapPlayer` lebih jauh dari constructor kalau
fitur mati. Kelas bug sama persis dengan WorkManager (Batch 387): resource mahal dibangun di
jalur startup yang provably tidak dibutuhkan buat konfigurasi paling umum.

**Fix**: `overlapPlayer`+`CrossfadeEngine` dipindah ke `ensureCrossfadeEngine(sessionPlayer)` —
early-return kalau sudah dibangun, kalau belum baru dibangun sekarang. Dipanggil dari `onCreate()`
(HANYA kalau Crossfade sudah ON sejak sebelum cold start — zero behavior change buat user ini) dan
dari `onCustomCommand ACTION_SET_CROSSFADE_ENABLED` saat `enabled==true` pertama kali sesi ini.
AudioAttributes overlapPlayer dibaca balik dari `sessionPlayer.audioAttributes` (bukan dibangun
ulang), tetap 100% konsisten dgn `player`. Detail lengkap + konsekuensi jujur (biaya konstruksi
kecil pindah ke titik toggle Settings buat yang baru menyalakan mid-session, bukan zero-cost
murni di SEMUA kasus seperti Batch 385/386) di CHANGELOG.md Batch 389.

**1 file kode disentuh** (`PlaybackService.kt`, `CrossfadeEngine.kt` TIDAK disentuh). Brace/paren/
bracket balance dicek (python3, 812 baris): seimbang. Diff-checked terhadap ZIP asli: cuma 1 file
berubah. **Belum ditest di device asli** (sama seperti Batch 385-387) — juga belum lewat build/
lint CI asli, tidak ada kotlinc/Android SDK/network di environment kerja sesi ini.

**Batch 388 (Audit lanjutan cold-start — 0 file kode, murni dokumentasi)** — User instruksi:
"next" (lanjutan sesi optimasi yang sama, Batch 385→386→387). **Status DISCONTINUED tetap
permanen tidak diubah** (per klarifikasi Batch 387).

Ditelusuri lebih lanjut sebelum menulis kode apa pun (bukan diasumsikan beres krn 3 batch
sebelumnya berhasil menemukan sesuatu):
- `Coil`'s `ImageLoaderFactory` — sudah lazy by design (Coil sendiri yang memanggil
  `newImageLoader()` on-demand saat `AsyncImage` pertama butuh loader, bukan dipaksa app ini
  sinkron di startup) — tidak ada yang perlu diubah.
- `LyricsDatabase` (Room, dipakai fitur lyrics cache) — singleton lazy standar
  (`getInstance()` double-checked locking), 3 call site digrep app-wide
  (`LyricsViewModel`/`SettingsScreen`/`LyricsPrefetchWorker`) — TIDAK ADA yang konstruksi dari
  `MainActivity`/`PlayerViewModel`/`AudioPlayerApplication`. Sudah 0 biaya cold-start dari awal,
  bukan sesuatu yang perlu di-defer lagi.
- `MediaController.Builder(...).buildAsync()` (`PlayerViewModel.connect()`) — sudah async by
  design (`ListenableFuture`), bukan blocking call.
- `installSplashScreen()` dipanggil tanpa `setKeepOnScreenCondition` custom — sudah pola
  tercepat (splash lepas otomatis begitu frame pertama digambar, tidak sengaja ditahan
  menunggu apa pun).
- Dependency lain yang dikenal biasa auto-init lewat App Startup (`androidx.emoji2`,
  `androidx.lifecycle` process-lifecycle, `androidx.profileinstaller` kalau ada) SENGAJA TIDAK
  disentuh — beda dari WorkManager (Batch 387) yang polanya bisa dibuktikan aman dari 1 call
  site tunggal yang digrep app-wide, ketiganya (a) dipakai luas oleh banyak
  komponen/library lain (matikan salah satu berisiko regresi diam-diam yang tidak kelihatan
  tanpa device asli), dan/atau (b) `profileinstaller` justru MEMPERCEPAT cold-start berikutnya
  by design (menonaktifkannya kontra-produktif, bukan optimasi).

**Kesimpulan jujur**: opportunity cold-start yang aman dibuktikan murni dari pembacaan kode
(tanpa akses device fisik) — kelas bug yang sama dengan Batch 385/386/387 (kerja sinkron/eager
yang provably tidak dibutuhkan sedini itu) — sudah habis utk pass ini. Kandidat yang tersisa
(cost riil Compose first-composition, setup ExoPlayer/MediaSession di `PlaybackService`,
baseline profile/R8 minification) SEMUANYA butuh salah satu dari: (a) data pengukuran device
asli (Macrobenchmark/systrace) utk tahu mana yang benar-benar dominan — menebak tanpa data
berisiko mengoptimasi bagian yang sebenarnya bukan bottleneck, atau (b) risiko regresi jauh
lebih tinggi drpd Batch 385-387 (mis. R8 di app yang 0/19 pernah di-QA device asli & sudah
discontinued — lihat § "Status penutupan (Batch 384)" poin 1). **Rekomendasi konkret utk sesi
berikutnya**: kalau user bisa akses device Android fisik, jalankan
`adb shell am start -W com.rudi.audioplayer/.MainActivity` (atau setup Macrobenchmark) SEBELUM
minta lanjut batch optimasi cold-start lagi — tanpa angka nyata, sesi berikutnya cuma bisa
mengulang kelas bug yang sama (kalaupun ada sisa) atau mulai menebak-nebak di area berisiko
lebih tinggi. **0 file kode disentuh batch ini** (murni audit + dokumentasi, konsisten pola
Batch 384 saat tidak ada kerja kode aman yang tersisa). Detail lengkap CHANGELOG.md Batch 388.
...
behavior tetap sesuai tujuan awal"), langsung di batch yang sama tanpa dipecah 2-3 batch seperti
perkiraan awal PENDING doc — investigasi kode nyata (bukan lagi perkiraan) menunjukkan cakupan
minimal SEBENARNYA cuma 4 file (bukan 6; 2 consumer minor `LyricsSheet.kt`/
`ABRepeatBookmarkSheet.kt` berhasil dihindari lewat wrapper composable lokal di
`NowPlayingScreen.kt`, lihat di bawah), dan memecahnya jadi 2 batch cuma akan menghasilkan batch
pertama yang 0 manfaat nyata (dual-write sementara tidak mengurangi frekuensi tick di `_uiState`
sama sekali sampai field lama benar-benar dihapus) — jadi dieksekusi atomik sekali jalan demi
`final behavior` yang benar-benar menuntaskan akar masalah, bukan separuh jalan.

**Root cause fix (bukan lagi mitigasi frekuensi seperti Batch 352)**: `position`/`duration`
dikeluarkan TOTAL dari `PlaybackUiState` (`PlayerViewModel.kt`), dipindah ke `StateFlow` baru
`PlaybackProgress(position, duration)` (`playbackProgress`) yang HANYA dikoleksi lokal oleh
composable kecil yang genuinely menampilkan posisi — bukan lagi di-hoist & dibaca utuh di scope
teratas `AppNavHost`. Mekanisme: recompose-scope Compose granular per pemanggilan fungsi
`@Composable` (termasuk lambda `content` yang dioper ke fungsi non-inline) — baca State di dalam
scope situ HANYA invalidate scope itu sendiri, tidak bocor ke scope pemanggil selama pemanggil
tidak ikut baca State yang sama. Jadi tick 1 detik (`startPositionLoop`, sudah 1000ms sejak Batch
352) sekarang betul-betul CUMA invalidate composable kecil itu — `AppNavHost` (Scaffold+NavHost+
tab switching+scroll) tidak lagi ikut ter-restart sama sekali tiap tick.

**Rincian 4 file**:
1. `PlayerViewModel.kt` — `PlaybackUiState` kehilangan field `position`/`duration`; `data class`
   baru `PlaybackProgress` + `StateFlow` `_playbackProgress`/`playbackProgress`.
   `startPositionLoop()` nulis ke `_playbackProgress` (bukan `_uiState`) dengan guard
   skip-kalau-sama yang persis sama seperti sebelumnya. **2 titik lain yang ikut break ditemukan
   via full-codebase sweep** (tidak ada di perkiraan awal PENDING doc, ketemu saat eksekusi) dan
   difix di batch yang sama: `onMediaItemTransition` (dulu reset `duration` lewat
   `_uiState.copy(...)`, sekarang lewat `_playbackProgress.copy(...)`) dan
   `setAudiobookModeEnabled` (dulu baca `_uiState.value.position`, sekarang
   `_playbackProgress.value.position`). Tanpa 2 fix ini project TIDAK akan compile — bukti nyata
   kenapa full sweep sebelum packaging itu wajib, bukan basa-basi.
2. `MiniPlayerBar.kt` — signature tambah param `playbackProgress: StateFlow<PlaybackProgress>`,
   `collectAsStateWithLifecycle()` LANGSUNG di dalam fungsi ini (fungsi ini sendiri sudah jadi
   batas scope yang pas, tidak perlu extract lagi).
3. `NowPlayingScreen.kt` — signature tambah param sama. Blok Slider+waveform+teks waktu
   di-extract jadi composable baru `PlaybackProgressRow` (collect lokal di situ, dipanggil dari
   body utama) — 1:1 sama tampilan/behavior, cuma pindah scope. `LyricsSheet`/
   `ABRepeatBookmarkSheet` (2 consumer minor `uiState.position`) TIDAK ikut diubah signature-nya
   (0 file tambahan) — dibungkus composable generik baru `WithLivePlaybackProgress` yang collect
   lokal lalu suplai `positionMs` lewat parameter lambda `content`, isolasi scope tanpa sentuh
   2 file itu sama sekali.
4. `MainActivity.kt` (**protected, edit-parsial**) — 2 baris tambahan SAJA di 2 call-site
   (`MiniPlayerBar(...)`/`NowPlayingScreen(...)`): `playbackProgress = playerViewModel.playbackProgress`.
   0 restrukturisasi `AppNavHost`, karena fungsi ini ternyata TIDAK PERNAH baca
   `uiState.position`/`.duration` langsung (cuma meneruskan `uiState` utuh ke 2 composable anak)
   — begitu field itu hilang dari `PlaybackUiState`, `AppNavHost` otomatis "sembuh" tanpa perlu
   disentuh strukturnya.

**Verifikasi sebelum packaging**: full-codebase grep untuk SEMUA sisa referensi
`uiState.position`/`.duration`/`_uiState.value.position`/`.duration` (0 tersisa di luar komentar
penjelasan) + brace/paren balance check di ke-4 file (0 selisih) + cek 0 file lain (Preview/test)
memanggil `MiniPlayerBar`/`NowPlayingScreen` yang perlu ikut diupdate. **Belum diverifikasi**:
compile CI sungguhan (sandbox 0 akses compiler) & lag hilang di device asli — WAJIB dikonfirmasi
user setelah build jalan, sesuai peringatan yang sama seperti Batch 351/352 (diagnosis dari audit
statis, bukan profiler).

**Status `PENDING_FixGlobalLagRecomposition.md`**: file DIHAPUS (bukan cuma diupdate) — kedua
opsi (A & B) sudah tuntas dieksekusi, tidak ada lagi yang genuinely "pending" untuk task ini;
riwayat lengkap sudah dipindah ke sini + `CHANGELOG.md`/`README.md`, tidak ada informasi yang
hilang.

**Ringkasan file** — 4 file kode disentuh (3 logic penuh: `PlayerViewModel.kt`,
`MiniPlayerBar.kt`, `NowPlayingScreen.kt`; 1 minim-diff protected: `MainActivity.kt`) + 3
dokumentasi VIP (PROJECT_STATE.md/CHANGELOG.md/README.md) + `PENDING_FixGlobalLagRecomposition.md`
dihapus. `FILE_MANIFEST.txt` diupdate 189→188 (0 file kode baru/hapus, 1 file dokumen
(`PENDING_FixGlobalLagRecomposition.md`) dihapus & dikeluarkan dari manifest — sudah dicocokkan
ulang `diff` manifest vs disk, 0 selisih).
...
Repeat lewat titik B naik ke maks ~1 detik (dari ~0.5 detik). SCOPE masalah utama tuntas di
Batch 353 di atas — catatan asli "AppNavHost masih recompose penuh" di bawah ini sudah TIDAK
BERLAKU lagi sejak Batch 353, dipertahankan apa adanya sebagai riwayat.

**Status PENDING_FixGlobalLagRecomposition.md (riwayat, sudah dihapus di Batch 353)**: diupdate
(bukan diarsipkan) — Opsi A masih berlaku penuh utk sesi berikutnya, root cause & rencana file
belum berubah sama sekali, cuma status header yang diubah dari "menunggu pilih" jadi "Opsi B
sudah jalan Batch 352, Opsi A masih nunggu confirm eksekusi".
