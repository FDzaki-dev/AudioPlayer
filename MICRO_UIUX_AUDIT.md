...
| 5 | Interactive States (disabled/selected/loading/error) | 🟡 **Berlanjut (Batch 162-165)**. 6/8 sub-item diperiksa: disabled icon-button tint (5 titik, konsisten), pressed/ripple integrity (0 `indication=null` ditemukan, aman), loading (`ShimmerBrush` shared, konsisten), **selected/active state (Batch 163-164)** — taksonomi 3 pola selected ditemukan by-design (Card preview→border+elevation; List/dialog option→RadioButton; Tag/filter→FilterChip fill), 2 konsisten (FilterChip 7 titik across EqualizerSheet/SmartPlaylistScreen/RingtoneCutterSheet; Card 1 titik ThemeOptionCard), **1 bug ditemukan & diperbaiki (Batch 163)**: `SpeedDialog`'s speed-list pakai TextButton+teks "✓" padahal `TransitionModeOption` di dialog SAMA persis (cuma dipisah 1 Divider) sudah RadioButton — disamakan. **1 dari 2 observasi tertunda DIEKSEKUSI (Batch 164)**: `SongRow` (LibraryScreen, 3 call site) sekarang menandai lagu yang sedang diputar (primary 12% bg + bold + ikon GraphicEq, disamakan persis pola `QueueRow`) — param `currentSongId` diwiring lewat `MainActivity.kt` (`uiState.currentSong?.id`, protected edit parsial 1 baris). **error state & success/confirmation feedback (Batch 165-166)** — teks validasi inline (4 titik: LockScreen/SettingsScreen/VaultSheet x2) 100% konsisten, 0 bug. Banner hasil-operasi (4 titik/3 file) ditemukan 3-arah tidak konsisten (Batch 165) → **DISATUKAN (Batch 166)**: composable shared `ResultBanner` (`Utils.kt`) + `enum ResultBannerStyle {Solid, Tinted, Bare}`, 3 gaya visual lama DIPERTAHANKAN lewat parameter (bukan dipaksa 1 tampilan) — `BackupRestoreSheet`+`DiagnosticLogSheet`→Solid, `SignatureMatcherSheet`→Tinted, `UpdateCheckSheet`→Bare (delegasi dari private `StatusBanner`, 0 call site berubah). 0 perubahan visual disengaja, akar masalah (implementasi triplikat gampang melenceng) yang diperbaiki, bukan tampilannya. **1 observasi masih tertunda keputusan user**: `LibraryFilterChips` custom Box primary-fill solid vs pola FilterChip secondaryContainer di tempat lain. Sisa kategori #5: konsistensi lintas-aksi lain (item terakhir, belum diperiksa). |
