...
| 2 | Spacing & Sizing Consistency | 🟡 **Berlanjut (Batch 146-147)**. 146: audit horizontal screen padding — 4 tab utama Home/Settings/NowPlaying/Playlist/SmartPlaylist/StatsDashboard sudah 20dp; 2 gap di `LibraryScreen.kt` diperbaiki (`AlbumGridView` 16dp→20dp horizontal, 4 titik `ListItem` Artis/Folder/hasil-pencarian/riwayat 4dp→20dp). 147: audit ukuran control setara — `LockScreen.kt` ikon Fingerprint(28dp)/Backspace(22dp) disamakan ke 24dp (komentar kode sendiri nyatakan "same treatment", 7 titik `Icon().size()` lain dicek & TIDAK disentuh, beda konteks/sudah konsisten). Pending: vertical spacing antar section, gap icon↔text (sebaran Spacer 4-16dp, butuh pengelompokan per-konteks dulu — terlalu luas buat sweep mekanis), sisa literal `.dp` lain. |
| 3 | Typography Hierarchy | 🟡 **Berlanjut (Batch 149)**. Audit title bottom sheet: grep 13 `ModalBottomSheet` header — 12 sudah konsisten `titleMedium`+Bold, 1 gap nyata (`FolderManagerSheet.kt` pakai `titleLarge` tanpa `fontWeight`) disamakan ke `titleMedium`+Bold. Pending: audit body/label/caption, line-height, cakupan penuh truncation/ellipsis (sebagian sudah Batch 37, belum formal utk kategori #3). |
