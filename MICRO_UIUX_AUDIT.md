...
| 3 | Typography Hierarchy | 🟡 **Berlanjut (Batch 149, 154, 159)**. 149: title bottom sheet (13 titik, 1 gap `FolderManagerSheet` disamakan). 154: body/label song-row dalam sheet (5 titik konsisten, 1 gap `FolderManagerSheet` "Lagu Disembunyikan" disamakan). 159: field-label caption di atas control (9 titik, 1 gap `SignatureMatcherSheet` `ApkPickerRow` disamakan ke `labelLarge`+default). Pending: badge/axis-label/nav-text (belum diaudit formal), line-height, cakupan penuh truncation/ellipsis (sebagian Batch 37, belum formal kategori #3). |
