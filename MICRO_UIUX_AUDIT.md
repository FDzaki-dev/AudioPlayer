...
| 1 | Strings & Wording Consistency | 🟡 **Dimulai (Batch 142)**. Wording undo-hide `LibraryScreen.kt` disamakan ke label kanonik `"Urungkan"` (sebelumnya `"Batalkan"`, beda dari standar `UndoableAction` di `MainActivity.kt`). Pending: audit formal `"Batal"` vs `"Tutup"` (20 titik), kapitalisasi/tanda baca title dialog, tulis hasil cek `"Hapus"` (22 titik, sudah dicek sekilas beda konteks). |
