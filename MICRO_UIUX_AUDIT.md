...
| 5 | Interactive States (disabled/selected/loading/error) | 🟡 **Berlanjut (Batch 162-163)**. 4/8 sub-item diperiksa: disabled icon-button tint (5 titik, konsisten), pressed/ripple integrity (0 `indication=null` ditemukan, aman), loading (`ShimmerBrush` shared, konsisten), **selected/active state (Batch 163)** — taksonomi 3 pola selected ditemukan by-design (Card preview→border+elevation; List/dialog option→RadioButton; Tag/filter→FilterChip fill), 2 konsisten (FilterChip 7 titik across EqualizerSheet/SmartPlaylistScreen/RingtoneCutterSheet; Card 1 titik ThemeOptionCard), **1 bug ditemukan & diperbaiki**: `SpeedDialog`'s speed-list pakai TextButton+teks "✓" padahal `TransitionModeOption` di dialog SAMA persis (cuma dipisah 1 Divider) sudah RadioButton — disamakan. **2 observasi tertunda keputusan user** (bukan bug pasti, butuh konfirmasi eksplisit sebelum eksekusi — pola sama Batch 162 EmptyState): (a) `LibraryFilterChips` (tab Lagu/Album/Artis + \"Lainnya\") custom Box primary-fill solid, BEDA dari pola FilterChip secondaryContainer di tempat lain — bisa jadi hierarki navigasi-primer vs filter-sekunder yang disengaja, bisa juga genuinely inkonsisten; (b) `SongRow` (LibraryScreen, 3 call site) TIDAK PERNAH menandai lagu yang sedang diputar, padahal `QueueSheet`'s row SUDAH (primary 12% bg + bold) — perbaikannya butuh parameter baru + wiring current-song state lewat `NavGraph`/`MainActivity` (protected, lintas-file), di luar cap batch kecil. Sisa kategori #5: empty state (icon-mismatch Batch 162), error state, success feedback, konsistensi lintas-aksi lain. |
